...
- `npm run test` — runs Vitest (integration tests in `src/lib/__tests__/` need MariaDB on `DATABASE_URL`; without DB, DB tests are skipped). Use `npm run test:ci` to fail if the database is unavailable (for CI).

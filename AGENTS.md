...
使用根目录 `eslint.config.mjs`（`eslint-config-next` 的扁平配置）与 `npm run lint`（`eslint .`）。不要使用已移除的 `.eslintrc.json` + `next lint`，否则会在 ESLint 9 下出现 “Converting circular structure to JSON”。生产构建仍可在 `next.config.ts` 中跳过 ESLint。

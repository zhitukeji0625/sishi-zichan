...
- `npm run test` — runs Vitest (integration tests in `src/lib/__tests__/`; skips DB tests if `DATABASE_URL` is unreachable)
- `npm run test:db` — same as above but fails if the database is unavailable (`VITEST_REQUIRE_DB=1`)

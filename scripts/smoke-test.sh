...
RENT_PROJECT_ID=$(cd "$(dirname "$0")/.." && npx tsx -e "
(async () => {
  ...
  if (!existing) {
    ...
    const amount = topBid?.amount ?? result.project.startPrice;
...
if [[ -n "$RENT_PROJECT_ID" ]]; then
  code=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/api/m/payments/mock" \
    -H "Content-Type: application/json" -b "$USER_COOKIE" \
    -d "{\"purpose\":\"AUCTION_RENT\",\"auctionProjectId\":\"$RENT_PROJECT_ID\"}")

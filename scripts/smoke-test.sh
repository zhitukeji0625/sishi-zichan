...
if [ -n "$LISTING_ID" ]; then
  read -r START END < <(node --import tsx/esm -e "
import { PrismaClient } from '@prisma/client';
const p = new PrismaClient();
(async () => {
  ...
  const existing = user
    ? await p.dryingReservation.findMany({
        where: { listingId, endUserId: user.id, status: { notIn: ['REJECTED', 'CANCELLED'] } },
        select: { startDate: true, endDate: true },
      })
    : [];
  const taken = new Set(existing.flatMap((r) => {
    const days = [];
  ...
  for (let offset = 1; offset <= 60; offset++) {
    ...
    const key2 = next.toISOString().slice(0, 10);
    if (!taken.has(key) && !taken.has(key2)) {
      start = key;
      process.stdout.write(key + ' ' + key2 + '\n');
      break;
    }
  }

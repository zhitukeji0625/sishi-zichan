...
# 9. Upload non-multipart returns 400 (authenticated)
check "upload non-multipart" "400" "$(curl -s -o /dev/null -w '%{http_code}' -b /tmp/smoke_admin.txt -X POST "$BASE/api/upload" -H 'Content-Type: application/json' -d '{"x":1}')"

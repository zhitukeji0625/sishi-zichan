...
if [[ -n "$LISTING_ID" ]]; then
  DAY_OFFSET=$(( $(date +%s) % 200 + 30 ))
  DRY_START=$(date -d "+${DAY_OFFSET} days" +%Y-%m-%d)
  DRY_END=$(date -d "+$((DAY_OFFSET + 1)) days" +%Y-%m-%d)
...
# --- Payment mock (duplicate deposit should be 409; seed user already paid) ---
if [[ -n "${PROJECT_ID:-}" ]]; then
  ...
  assert_status "POST /api/m/payments/mock (duplicate deposit)" "409" "$code"
fi

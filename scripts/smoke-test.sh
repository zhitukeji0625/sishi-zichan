...
(async () => {
  ...
  if (!topBid) { await p.\$disconnect(); return; }
  await p.auctionResult.upsert({
    where: { projectId },
    create: { projectId, winnerId: user.id, status: 'APPROVED' },
    update: { winnerId: user.id, status: 'APPROVED' },
  });
  await p.payment.upsert({
    where: { orderNo: 'SMOKE_RENT_PAID' },
    create: {
      orderNo: 'SMOKE_RENT_PAID',

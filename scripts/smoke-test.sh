...
# 13 Dict data present (Prisma count)
...
# 17 Assets create non-multipart returns 400
ASSET_CODE=$(curl -s -b "$ADMIN_COOKIE" -o /dev/null -w "%{http_code}" -X POST "$BASE/api/admin/assets" -H "Content-Type: application/json" -d '{}')
if [ "$ASSET_CODE" = "400" ]; then pass "assets rejects non-multipart (400)"; else fail "assets non-multipart (got $ASSET_CODE)"; fi

# 18 Bid on live auction (dynamic min amount)
if [ -n "$PROJECT_ID" ]; then

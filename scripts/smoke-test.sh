...
# 12. Auction bid API (requires LIVE auction + authenticated user)
BID_INFO=$(cd "$(dirname "$0")/.." && npx tsx -e "
import { PrismaClient } from '@prisma/client';
import { Decimal } from '@prisma/client/runtime/library';
(async () => {
  const p = new PrismaClient();
  const project = await p.auctionProject.findFirst({ where: { status: 'LIVE' }, select: { id: true, startPrice: true, bidStep: true } });
  if (!project) { console.log(''); await p.\$disconnect(); return; }
  const top = await p.auctionBid.findFirst({ where: { projectId: project.id }, orderBy: { amount: 'desc' }, select: { amount: true } });
  const min = top
...
if [[ -n "$AUCTION_ID" && -n "$BID_AMOUNT" ]]; then
  BID_RES=$(curl -s -X POST "$BASE/api/m/auction/${AUCTION_ID}/bid" \
    -H 'Content-Type: application/json' \
    -b /tmp/smoke_user.txt \
    -d "{\"amount\":${BID_AMOUNT}}" \
    -w '%{http_code}')
  BID_CODE="${BID_RES: -3}"
  check "auction bid API" "200" "$BID_CODE"
else
  echo "  ✗ auction bid API (no LIVE auction in database)"
  FAIL=$((FAIL + 1))
fi

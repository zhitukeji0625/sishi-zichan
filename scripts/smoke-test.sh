...
PROJ_JSON=$(cd "$(dirname "$0")/.." && npx tsx -e "(async()=>{const{PrismaClient}=await import('@prisma/client');const p=new PrismaClient();const r=await p.auctionProject.findFirst({where:{status:'LIVE'},select:{id:true,startPrice:true}});console.log(JSON.stringify(r));await p.\$disconnect();})()")
PROJ_ID=$(echo "$PROJ_JSON" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d['id'] if d else '')")
START_PRICE=$(echo "$PROJ_JSON" | python3 -c "import sys,json; d=json.load(sys.stdin); print(int(float(d['startPrice'])) if d else 8000)")

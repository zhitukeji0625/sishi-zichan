...
# Refresh demo auction via list page, then bid with dynamic minimum
curl -s "$BASE/m/auction" > /dev/null
read -r PROJ_ID BID_AMT < <(cd "$(dirname "$0")/.." && ./node_modules/.bin/tsx -e "
(async()=>{
...
LISTING_ID=$(cd "$(dirname "$0")/.." && ./node_modules/.bin/tsx -e "(async()=>{const{PrismaClient}=await import('@prisma/client');const p=new PrismaClient();const r=await p.dryingFieldListing.findFirst({where:{status:'OPERATING'},select:{id:true}});console.log(r?.id??'');await p.\$disconnect();})()")
...
code=$(curl -s -o /tmp/smoke-r15 -w "%{http_code}" "$BASE/m/auction")
check "auction page" "200" "$code" "$(head -c 100 /tmp/smoke-r15)"

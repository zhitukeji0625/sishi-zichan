...
# 9. Upload without multipart should return 400
check "upload no multipart" "400" "$(curl -s -o /dev/null -w '%{http_code}' -b /tmp/smoke_admin.txt -X POST "$BASE/api/upload" -H 'Content-Type: application/json' -d '{}')"

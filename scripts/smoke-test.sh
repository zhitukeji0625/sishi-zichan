...
(async () => {
  const { PrismaClient } = await import('@prisma/client');
  const p = new PrismaClient();
  const user = await p.endUser.findFirst({ where: { phone: '13800138000' } });
  if (!user) { console.log(''); process.exit(0); }
  const result = await p.auctionResult.findFirst({ where: { winnerId: user.id } });
  if (!result) { console.log(''); process.exit(0); }
  const existing = await p.payment.findFirst({
    where: { auctionProjectId: result.projectId, endUserId: user.id, purpose: 'AUCTION_RENT', status: 'SUCCESS' },
  });
  if (!existing) {
...
if [[ -n "$RENT_PROJECT" ]]; then
  ...
  echo "  ⚠ skip duplicate rent — no winner project for demo user"
fi

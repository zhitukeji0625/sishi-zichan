...
(async () => {
  ...
  if (!user) { await p.\$disconnect(); return; }
  await p.auctionResult.upsert({
    where: { projectId },
    update: { winnerId: user.id, status: 'PUBLISHED' },
    create: { projectId, winnerId: user.id, status: 'PUBLISHED', publishedAt: new Date() },
  });

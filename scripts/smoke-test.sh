...
WINNER_PROJECT_ID=$(cd "$(dirname "$0")/.." && npx tsx -e "
(async () => {
  const { PrismaClient } = await import('@prisma/client');
  const p = new PrismaClient();
  const user = await p.endUser.findFirst({ where: { phone: '13800138000' }, select: { id: true } });
  ...
  if (!result) { console.log(''); process.exit(0); }
  const paid = await p.payment.findFirst({
    where: {
  ...
  if (!paid) {
    const topBid = await p.auctionBid.findFirst({
      where: { projectId: result.projectId, endUserId: user.id },
      orderBy: { amount: 'desc' },
    });
    const amount = topBid?.amount ?? 8000;
    await p.payment.create({
      data: {
        orderNo: 'SMOKE' + Date.now(),
        amount,
        purpose: 'AUCTION_RENT',
        status: 'SUCCESS',
        endUserId: user.id,
...
if [[ -n "$WINNER_PROJECT_ID" ]]; then
  code=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/api/m/payments/mock" \
    -H "Content-Type: application/json" -b "$USER_COOKIE" \
    -d "{\"purpose\":\"AUCTION_RENT\",\"auctionProjectId\":\"$WINNER_PROJECT_ID\"}")
  assert_status "POST /api/m/payments/mock (duplicate rent)" "409" "$code"
else
  echo "  ⚠ skip duplicate rent payment — no winner project for demo user"
fi

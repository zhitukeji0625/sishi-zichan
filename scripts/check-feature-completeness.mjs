...
/** 将连续汉字拆成 2～maxLen 字的子串，避免「系统管理账号管理…」整段无法匹配「管理员账号」等短语 */
function addChineseSubstrings(run, set, minLen = 2, maxLen = 24) {
  if (run.length < minLen) return;
  const n = run.length;
  const cap = Math.min(maxLen, n);
  for (let i = 0; i < n; i++) {
    for (let len = minLen; len <= cap && i + len <= n; len++) {
      set.add(run.slice(i, i + len));
    }
  }
}

function collectCnTokensFromText(blob) {
  const set = new Set();
  let run = "";
...
function collectKeywords(row) {
  const segments = [
    ...
    row.mod2,
    row.feature,
    `${row.mod1}${row.mod2}${row.feature}`,
  ];
  const cnSet = new Set();
  for (const seg of segments) {
    const normalized = String(seg).replace(/\//g, "");
    for (const t of normalized.match(CN_TOKEN) ?? []) cnSet.add(t);
    for (const t of collectCnTokensFromText(normalized)) cnSet.add(t);
  }
  const cn = [...cnSet];
  const en = new Set();
  for (const t of cn) {

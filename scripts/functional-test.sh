...
if [ -z "$PROJECT_ID" ]; then
  ...
  BID_INFO=$(query_db "const proj=await p.auctionProject.findFirst({where:{code:'DEMO_LIVE_AUCTION',status:'LIVE'}});const top=await p.auctionBid.findFirst({where:{projectId:proj?.id},orderBy:{amount:'desc'}});const min=top?Number(top.amount)+Number(proj.bidStep):Number(proj.startPrice);console.log(JSON.stringify({amount:min}));")
  BID_AMOUNT=$(echo "$BID_INFO" | python3 -c "import sys,json; print(json.load(sys.stdin)['amount'])")

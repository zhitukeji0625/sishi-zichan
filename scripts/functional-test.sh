...
if [ -z "$PROJECT_ID" ]; then
  ...
  BID_AMOUNT=$(query_db "const proj=await p.auctionProject.findFirst({where:{code:'DEMO_LIVE_AUCTION',status:'LIVE'}});const top=await p.auctionBid.findFirst({where:{projectId:proj?.id},orderBy:{amount:'desc'}});const base=top?Number(top.amount):Number(proj?.startPrice??8000);const step=Number(proj?.bidStep??200);console.log(base+step);")

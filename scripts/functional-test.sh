...
if [ -z "$PROJECT_ID" ]; then
  ...
  BID_AMOUNT=$(query_db "const proj=await p.auctionProject.findUnique({where:{id:'$PROJECT_ID'}});const top=await p.auctionBid.findFirst({where:{projectId:'$PROJECT_ID'},orderBy:{amount:'desc'}});const base=top?Number(top.amount):Number(proj?.startPrice??8000);const step=Number(proj?.bidStep??200);console.log(base+step);")

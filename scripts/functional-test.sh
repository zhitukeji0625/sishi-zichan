...
if [ -z "$PROJECT_ID" ]; then
  ...
  MIN_BID=$(query_db "const proj=await p.auctionProject.findUnique({where:{id:'$PROJECT_ID'}});const top=await p.auctionBid.findFirst({where:{projectId:'$PROJECT_ID'},orderBy:{amount:'desc'}});if(!proj){console.log('');}else{const{Decimal}=await import('@prisma/client/runtime/library');const min=top?new Decimal(top.amount.toString()).plus(proj.bidStep.toString()):new Decimal(proj.startPrice.toString());console.log(min.toString());}")

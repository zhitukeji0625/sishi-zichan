...
export async function upsertEndUserFromExternal(
  ...
  if (existing) {
    const updateData: { name: string; orgId?: string; phone?: string } = {
      name: profile.displayName,
      ...(orgId ? { orgId } : {}),
    };
    if (profile.phone) {
      const phoneTaken = await prisma.endUser.findUnique({ where: { phone: profile.phone } });
      if (!phoneTaken || phoneTaken.id === existing.endUserId) {

...
export async function validateBookingAdvance(
  ...
  const rules = await prisma.dryingBookingRule.findFirst({ where: { listingId } });
  const maxDays = rules?.maxAdvanceDays ?? 7;
  const today = startOfDay(new Date());
  const maxDate = addDays(today, maxDays);
  if (startOfDay(start) < today) {
    return { ok: false, message: "开始日期不能早于今天" };
  }
  if (startOfDay(end) > maxDate || startOfDay(start) > maxDate) {
    return { ok: false, message: `最多提前 ${maxDays} 天预约` };
  }
  return { ok: true };
}
...
export async function validateReservationRange(
  ...
  const advance = await validateBookingAdvance(listingId, start, end);

...
/** Same user must not have overlapping active reservations on one listing. */
export async function findUserOverlappingReservation(
  ...
  return existing.find((r) => rangesOverlap(start, end, r.startDate, r.endDate));
}
...
export async function validateReservationRange(
  ...
  for (const day of days) {
    ...
    if (available <= 0) {
      return {
        ok: false,
        message: `${format(day, "yyyy-MM-dd")} 已满`,
        conflict: true,
      };
    }
  }

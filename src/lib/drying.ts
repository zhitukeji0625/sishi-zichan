...
/** Inclusive day range; returns [] when end is before start. */
export function daysInReservationRange(start: Date, end: Date): Date[] {
...
export async function getCapacityForDay(listingId: string, day: Date) {
  ...
  for (const r of reservations) {
    const days = daysInReservationRange(r.startDate, r.endDate);
...
export async function validateReservationRange(
  ...
  const days = daysInReservationRange(start, end);
  if (days.length === 0) {

...
export function computeMinimumBidAmount(
  highestBid: { amount: Decimal } | null,
  startPrice: Decimal,
  bidStep: Decimal,
): Decimal {
  if (highestBid) {
    return new Decimal(highestBid.amount.toString()).plus(bidStep.toString());
  }
...
export async function placeBid(params: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const minNext = computeMinimumBidAmount(top, project.startPrice, project.bidStep);

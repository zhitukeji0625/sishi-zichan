...
/** 下一口最低可接受出价（无历史出价时为起拍价，否则为当前最高价加价幅）。 */
export function minNextBidAmount(params: {
  startPrice: Decimal | string | number;
  bidStep: Decimal | string | number;
  topAmount: Decimal | string | number | null;
}) {
  const { startPrice, bidStep, topAmount } = params;
  if (topAmount == null) {
    return new Decimal(startPrice.toString());
  }
  return new Decimal(topAmount.toString()).plus(bidStep.toString());
}

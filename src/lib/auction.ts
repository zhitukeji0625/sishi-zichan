...
export function minNextBidAmount(params: {
  highestAmount: string | null | undefined;
  startPrice: Decimal;
  bidStep: Decimal;
}): Decimal {
  const { highestAmount, startPrice, bidStep } = params;
  return highestAmount != null && highestAmount !== ""
    ? new Decimal(highestAmount).plus(bidStep.toString())
...
export async function placeBid(params: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const minNext = minNextBidAmount({
      highestAmount: top?.amount != null ? top.amount.toString() : null,

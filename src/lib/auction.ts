...
/** 下一笔有效出价必须达到的最低金额（无历史出价时为起拍价）。 */
export function minNextRequiredBid(
  startPrice: Decimal,
  bidStep: Decimal,
...
export async function placeBid(params: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const minNext = minNextRequiredBid(
      new Decimal(project.startPrice.toString()),
      new Decimal(project.bidStep.toString()),
      top ? new Decimal(top.amount.toString()) : null,
    );

...
/** 无数据库依赖：根据起拍价、加价步长与当前最高价计算下一次最低可出价 */
export function minNextBidAmount(args: {
  ...
  const step = new Decimal(args.bidStep.toString());
  const start = new Decimal(args.startPrice.toString());
  const top = args.highestBidAmount;
  if (top == null) {
    return start;
  }

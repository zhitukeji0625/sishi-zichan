...
/** 下一笔有效出价不得低于该金额（无历史出价时为起拍价，否则为当前最高价加步长）。 */
export function getMinRequiredBidAmount(
  startPrice: Decimal,
  bidStep: Decimal,
...
export async function placeBid(params: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const highest = top ? new Decimal(top.amount.toString()) : null;
    const minNext = getMinRequiredBidAmount(project.startPrice, project.bidStep, highest);

...
export function nextMinimumBidAmount(
  highest: Decimal | null | undefined,
  startPrice: Decimal,
  bidStep: Decimal,
): Decimal {
  return highest
    ? new Decimal(highest.toString()).plus(new Decimal(bidStep.toString()))
...
export async function placeBid(params: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const minNext = nextMinimumBidAmount(top?.amount, project.startPrice, project.bidStep);

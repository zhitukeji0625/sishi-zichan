...
/** 下一口最低有效价：无出价时为起拍价，否则为当前最高价加价幅。便于单测且不依赖数据库。 */
export function getMinNextBidAmount(
  project: { startPrice: Decimal | string | number; bidStep: Decimal | string | number },
...
export async function placeBid(params: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const minNext = getMinNextBidAmount(project, top?.amount ?? null);

...
describe.skipIf(skipDb)("placeBid", () => {
  ...
  it("resolveAuctionProjectId finds by id and code", async () => {
    const project = await prisma.auctionProject.findUnique({ where: { id: projectId } });
    expect(project).not.toBeNull();
    expect(await resolveAuctionProjectId(projectId)).toBe(projectId);
    expect(await resolveAuctionProjectId(project!.code)).toBe(projectId);
    expect(await resolveAuctionProjectId("not-a-project")).toBeNull();
  });
});

...
describe.skipIf(skipDb)("validateBookingDates", () => {
  ...
  it("rejects past start dates", async () => {
    ...
    const result = await validateBookingDates(listingId, past, past);
  ...
  it("rejects dates beyond max advance days", async () => {
    ...
    const result = await validateBookingDates(listingId, tooFar, tooFar);
    expect(result.ok).toBe(false);
  ...
  it("accepts valid dates within range", async () => {
    ...
    const result = await validateBookingDates(listingId, tomorrow, tomorrow);

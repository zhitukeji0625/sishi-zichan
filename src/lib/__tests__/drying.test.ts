...
describe.skipIf(skipDb)("validateReservationRange", () => {
  ...
  it("rejects dates beyond maxAdvanceDays", async () => {
    ...
    if (!result.ok) {
      expect(result.message).toContain("最多只能提前 7 天预约");
    }
  });

  it("accepts dates within maxAdvanceDays", async () => {
    const within = addDays(new Date(), 3);
    const result = await validateReservationRange(listingId, within, within);
  ...
  it("rejects start dates before today", async () => {
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.message).toContain("不能早于今天");
    }
  });
});

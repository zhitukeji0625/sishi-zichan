...
/** Validate org exists and admin may access it. */
export async function validateAdminOrgAccess(
  ...
  const can = await adminCanAccessOrg(role, adminOrgId, targetOrgId);
  if (!can) return { ok: false, error: "无权在该组织录入资产", status: 403 };
  return { ok: true };
}

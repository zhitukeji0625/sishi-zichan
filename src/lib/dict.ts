...
const builtinDictMaps: Record<string, Record<string, string>> = {
...
function builtinDictItems(categoryCode: string): DictOption[] {
  const map = builtinDictMaps[categoryCode];
  if (!map) return [];
  return Object.entries(map).map(([value, label], sortOrder) => ({
    value,
    label,
    sortOrder,
  }));
...
export async function getDictItems(categoryCode: string): Promise<DictOption[]> {
  ...
  return builtinDictItems(categoryCode).map(({ value, label }) => ({ value, label }));
}

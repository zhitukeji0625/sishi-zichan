...
import { assetTypeLabels, assetStatusLabels, adminRoleLabels, orgLevelLabels, auctionStatusLabels, registrationStatusLabels, announcementStatusLabels, dryingListingStatusLabels, reservationStatusLabels, contractTypeLabels, contractStatusLabels, paymentPurposeLabels, paymentStatusLabels, endUserTypeLabels } from "@/lib/labels";
...
export async function getDictItems(categoryCode: string): Promise<DictOption[]> {
  ...
  const fallback = builtInFallback[categoryCode];
  if (fallback) {

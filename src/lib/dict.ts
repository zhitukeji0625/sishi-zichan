...
const staticDictMaps: Record<string, Record<string, string>> = {
...
function staticDictItems(categoryCode: string): DictOption[] {
  const map = staticDictMaps[categoryCode];
  if (!map) return [];
...
export async function getDictItems(categoryCode: string): Promise<DictOption[]> {
  ...
  return staticDictItems(categoryCode);
}

...
const BUILT_IN_DICTS: Record<string, Record<string, string>> = {
...
function builtInOptions(categoryCode: string): DictOption[] {
  const map = BUILT_IN_DICTS[categoryCode];
  if (!map) return [];
...
export async function getDictItems(categoryCode: string): Promise<DictOption[]> {
  ...
  if (!cat || cat.items.length === 0) return builtInOptions(categoryCode);

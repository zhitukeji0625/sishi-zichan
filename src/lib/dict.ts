...
const BUILTIN_DICT: Record<string, Record<string, string>> = {
...
function builtinDictItems(categoryCode: string): DictOption[] {
  const map = BUILTIN_DICT[categoryCode];
  if (!map) return [];
...
export async function getDictItems(categoryCode: string): Promise<DictOption[]> {
  ...
  if (!cat || cat.items.length === 0) return builtinDictItems(categoryCode);

...
const BUILTIN_DICT_FALLBACKS: Record<string, Record<string, string>> = {
...
function fallbackItems(categoryCode: string): DictOption[] {
  const map = BUILTIN_DICT_FALLBACKS[categoryCode];
  if (!map) return [];
...
export async function getDictItems(categoryCode: string): Promise<DictOption[]> {
  ...
  if (!cat) return fallbackItems(categoryCode);
  if (cat.items.length === 0) return fallbackItems(categoryCode);

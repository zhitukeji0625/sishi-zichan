...
const builtInFallbacks: Record<string, Record<string, string>> = {
...
function fallbackMap(categoryCode: string): Record<string, string> {
  return builtInFallbacks[categoryCode] ?? {};
}
...
export async function getDictItems(categoryCode: string): Promise<DictOption[]> {
  ...
  const fallback = fallbackMap(categoryCode);

...
/** 数据库字典缺失时的内置回退（与 seed-dict / labels 一致） */
const BUILT_IN_DICT: Record<string, Record<string, string>> = {
...
function builtInOptions(categoryCode: string): DictOption[] {
  const map = BUILT_IN_DICT[categoryCode];
  if (!map) return [];
...
export async function getDictItems(categoryCode: string): Promise<DictOption[]> {
  ...
  return builtInOptions(categoryCode);
}

...
const BUILTIN_LABELS: Record<string, Record<string, string>> = {
...
function builtinDictItems(categoryCode: string): DictOption[] {
  const map = BUILTIN_LABELS[categoryCode];
  if (!map) return [];
...
export async function getDictItems(categoryCode: string): Promise<DictOption[]> {
  ...
  if (!cat) return builtinDictItems(categoryCode);
  if (cat.items.length === 0) return builtinDictItems(categoryCode);

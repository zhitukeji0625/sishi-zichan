...
const STATIC_FALLBACK: Record<string, Record<string, string>> = {
...
function fallbackItems(categoryCode: string): DictOption[] {
  const map = STATIC_FALLBACK[categoryCode];
  if (!map) return [];
...
export async function getDictItems(categoryCode: string): Promise<DictOption[]> {
  ...
  if (!cat?.items.length) return fallbackItems(categoryCode);
...
export async function getDictMap(
  ...
  return { ...STATIC_FALLBACK[categoryCode] };
}

...
export default async function AuctionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const raw = project.asset.imagesJson ? JSON.parse(project.asset.imagesJson) : [];
    if (Array.isArray(raw)) {
      imageUrls = raw.filter((u): u is string => typeof u === "string" && u.length > 0);
    }
  } catch {
    imageUrls = [];
  }

  const assetImageGallery: ReactNode =
    imageUrls.length > 0 ? (

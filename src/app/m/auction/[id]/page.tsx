...
import { getHighestBid, minNextBidAmount } from "@/lib/auction";
...
export default async function AuctionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (project.asset.imagesJson) {
    try {
      const parsed = JSON.parse(project.asset.imagesJson);
      galleryUrls = Array.isArray(parsed)
      ...
      <div className="relative -mt-6 px-4 space-y-4">
        ...
        <div className="space-y-3 animate-slide-up stagger-2">
          ...
          {user && reg?.status === "APPROVED" && reg.depositPaid && project.status === "LIVE" && (() => {
            const minNextDec = minNextBidAmount({
              topAmount: top,
              bidStep: project.bidStep,
              startPrice: project.startPrice,
            });
            const minNext = Number(minNextDec.toString());

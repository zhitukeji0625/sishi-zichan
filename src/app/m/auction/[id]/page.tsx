...
import { getHighestBid, nextMinimumBidAmount } from "@/lib/auction";
...
export default async function AuctionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const galleryUrls = parseAssetImagesJson(project.asset.imagesJson);
      ...
      <div className="relative -mt-6 px-4 space-y-4">
        ...
        <div className="space-y-3 animate-slide-up stagger-2">
          ...
          {user && reg?.status === "APPROVED" && reg.depositPaid && project.status === "LIVE" && (() => {
            const minNext = nextMinimumBidAmount({
              highestBidAmount: top ? new Decimal(top.toString()) : null,
              startPrice: new Decimal(project.startPrice.toString()),
              bidStep: new Decimal(project.bidStep.toString()),
            });
            return <BidForm projectId={projectId} minBid={Number(minNext.toString())} />;
          })()}

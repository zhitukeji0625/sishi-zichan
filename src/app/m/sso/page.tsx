...
function SsoInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!res.ok) {
        ...
        setErrMsg(typeof j.error === "string" ? j.error : "登录失败");
        return;
      }
      router.replace("/m");
      router.refresh();
  ...
  if (errMsg) {
    return <div className="px-4 pt-16 text-center text-sm text-slate-600">{errMsg}</div>;
  }
  return (
    <div className="px-4 pt-16 text-center text-sm text-slate-600">正在验证第三方票据…</div>
  );
}

...
export async function toggleDryingListingStatusAction(formData: FormData) {
  ...
  if (!ok) return { error: "无权操作" };
  const allowed = new Set<string>(Object.values($Enums.DryingListingStatus));
  if (!allowed.has(newStatus)) return { error: "无效状态" };
  await prisma.dryingFieldListing.update({
    where: { id: listingId },
    data: { status: newStatus as $Enums.DryingListingStatus },
  });

...
export async function toggleDryingListingStatusAction(formData: FormData) {
  ...
  if (!ok) return { error: "无权操作" };
  const allowed = new Set<string>(Object.values(DryingListingStatus));
  if (!allowed.has(newStatusRaw)) return { error: "无效状态" };
  const newStatus = newStatusRaw as DryingListingStatus;

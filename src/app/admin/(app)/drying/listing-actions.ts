...
export async function createDryingListingAction(formData: FormData) {
  ...
  const maxAdvanceDays = Math.max(0, Math.min(365, Number(formData.get("maxAdvanceDays") ?? 7) || 7));
  const maxPeople = Math.max(1, Math.min(999, Number(formData.get("maxPeople") ?? 10) || 10));

...
export async function toggleDryingListingStatusAction(formData: FormData) {
  ...
  if (!ok) return { error: "无权操作" };
  const statusValues: DryingListingStatus[] = ["OPERATING", "MAINTENANCE", "PAUSED", "OFFLINE"];
  if (!statusValues.includes(newStatus as DryingListingStatus)) return { error: "无效状态" };

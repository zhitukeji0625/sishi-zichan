...
export async function createDryingListingAction(formData: FormData) {
  ...
  if (!Number.isFinite(maxAdvanceDays) || maxAdvanceDays < 0 || maxAdvanceDays > 365) {
    return { error: "提前预约天数需在 0–365 之间" };
  }
  if (!Number.isFinite(maxPeople) || maxPeople < 1 || maxPeople > 999) {
    return { error: "每日最大人数需在 1–999 之间" };
  }

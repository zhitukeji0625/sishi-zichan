...
export async function reviewAuctionResultAction(formData: FormData) {
  ...
  if (approve) {
    ...
    // Refund non-winner deposits (when there is a winner, exclude them; otherwise refund all)
    const regWhere = {
      projectId: result.projectId,
      depositPaid: true,
      ...(result.winnerId ? { endUserId: { not: result.winnerId } } : {}),
    };
    const allRegs = await prisma.auctionRegistration.findMany({ where: regWhere });

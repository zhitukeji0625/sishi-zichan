...
export async function POST(req: Request) {
  ...
  const formParsed = await parseMultipartForm(req);
  if ("error" in formParsed) return NextResponse.json({ error: formParsed.error }, { status: 400 });
  const formData = formParsed;

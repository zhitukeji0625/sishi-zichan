...
import { parseMultipartForm } from "@/lib/form-data";
...
export async function POST(
  ...
  const formResult = await parseMultipartForm(req);
  if ("error" in formResult) return NextResponse.json({ error: formResult.error }, { status: 400 });
  const raw = Object.fromEntries(formResult.entries());

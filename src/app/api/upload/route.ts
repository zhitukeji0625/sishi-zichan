...
import { parseFormData } from "@/lib/http/form";
...
export async function POST(req: Request) {
  ...
  const formDataOrRes = await parseFormData(req);
  if (formDataOrRes instanceof NextResponse) return formDataOrRes;
  const formData = formDataOrRes;

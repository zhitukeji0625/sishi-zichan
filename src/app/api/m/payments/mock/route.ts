...
export async function POST(req: Request) {
  ...
  if (purpose === "AUCTION_DEPOSIT") {
    ...
    if (reg.status !== "APPROVED") {
      return NextResponse.json({ error: "报名未通过审核" }, { status: 400 });
    }
  ...
  const pay = await prisma.$transaction(async (tx) => {
    ...
    if (purpose === "AUCTION_RENT" && auctionProjectId) {
      const existing = await tx.payment.findFirst({
        where: { auctionProjectId, endUserId: user.id, purpose: "AUCTION_RENT", status: "SUCCESS" },
      });
      if (existing) throw new Error("DUPLICATE");
    }
    if (purpose === "DRYING_RENT" && reservationId) {
      const existing = await tx.payment.findFirst({
        where: { reservationId, endUserId: user.id, purpose: "DRYING_RENT", status: "SUCCESS" },
      });
      if (existing) throw new Error("DUPLICATE");
    ...
    if (e instanceof Error && e.message === "DUPLICATE") return null;
    throw e;
  });

  if (!pay) {
    return NextResponse.json({ error: "已支付" }, { status: 409 });
  }

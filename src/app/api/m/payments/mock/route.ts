...
export async function POST(req: Request) {
  ...
  if (purpose === "AUCTION_DEPOSIT") {
    ...
    if (reg.depositPaid) return NextResponse.json({ ok: true, alreadyPaid: true });
    ...
    if (existingRent) return NextResponse.json({ ok: true, alreadyPaid: true });
    ...
    if (existingDeposit) return NextResponse.json({ ok: true, alreadyPaid: true });

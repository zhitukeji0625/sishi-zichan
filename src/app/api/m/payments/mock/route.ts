...
export async function POST(req: Request) {
  ...
  if (purpose === "AUCTION_DEPOSIT") {
    ...
    if (reg.depositPaid) return NextResponse.json({ ok: true, orderNo: "already_paid" });
    ...
    if (existingRent) return NextResponse.json({ ok: true, orderNo: existingRent.orderNo });
    ...
    if (existingDeposit) return NextResponse.json({ ok: true, orderNo: existingDeposit.orderNo });

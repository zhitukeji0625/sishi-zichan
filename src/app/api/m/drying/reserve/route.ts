...
export async function POST(req: Request) {
  ...
  const latestBookable = startOfDay(addDays(new Date(), maxAdvance));
  if (startOfDay(start) > latestBookable || startOfDay(end) > latestBookable) {
    return NextResponse.json({ error: `只能预约未来 ${maxAdvance} 天内` }, { status: 400 });
  }

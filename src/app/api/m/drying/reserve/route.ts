...
import { getMaxAdvanceDays, validateAdvanceWindow, validateReservationRange } from "@/lib/drying";
...
export async function POST(req: Request) {
  ...
  const maxAdvanceDays = await getMaxAdvanceDays(parsed.data.listingId);
  const advanceCheck = validateAdvanceWindow(start, maxAdvanceDays);
  if (!advanceCheck.ok) {

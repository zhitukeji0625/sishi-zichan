...
import { validateReservationRange, validateBookingDates, parseLocalDate } from "@/lib/drying";
...
export async function POST(req: Request) {
  ...
  const bookingCheck = await validateBookingDates(parsed.data.listingId, start, end);
  if (!bookingCheck.ok) {
    return NextResponse.json({ error: bookingCheck.message }, { status: 400 });
  }

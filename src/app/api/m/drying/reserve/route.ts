...
import { validateAdvanceDays, validateReservationRange } from "@/lib/drying";
...
export async function POST(req: Request) {
  ...
  const advanceCheck = await validateAdvanceDays(parsed.data.listingId, start, end);
  if (!advanceCheck.ok) {
  ...
  try {
    res = await prisma.$transaction(async (tx) => {
      const check = await validateReservationRange(parsed.data.listingId, start, end, tx);

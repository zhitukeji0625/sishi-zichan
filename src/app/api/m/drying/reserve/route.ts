...
import { validateReservationRange, findUserOverlappingReservation } from "@/lib/drying";
...
export async function POST(req: Request) {
  ...
  const overlap = await findUserOverlappingReservation(parsed.data.listingId, user.id, start, end);
  if (overlap) {
    return NextResponse.json({ error: "您在该时段已有预约，请勿重复提交" }, { status: 409 });
  }

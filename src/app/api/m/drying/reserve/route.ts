...
import { validateReservationRange, hasOverlappingReservation } from "@/lib/drying";
...
export async function POST(req: Request) {
  ...
  if (await hasOverlappingReservation(parsed.data.listingId, user.id, start, end)) {
    return NextResponse.json({ error: "该时段已有预约" }, { status: 409 });
  }

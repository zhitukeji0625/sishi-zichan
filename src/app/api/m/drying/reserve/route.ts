...
import { validateReservationRange, validateUserReservationOverlap } from "@/lib/drying";
...
export async function POST(req: Request) {
  ...
  const userOverlap = await validateUserReservationOverlap(user.id, start, end);
  if (!userOverlap.ok) {
    return NextResponse.json({ error: userOverlap.message }, { status: 400 });
  }

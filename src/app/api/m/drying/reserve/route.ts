...
import { validateReservationRange, hasOverlappingUserReservation } from "@/lib/drying";
...
export async function POST(req: Request) {
  ...
  if (await hasOverlappingUserReservation(parsed.data.listingId, user.id, start, end)) {
    return NextResponse.json({ error: "您已有重叠时段的预约" }, { status: 400 });
  }

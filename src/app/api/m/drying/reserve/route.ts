...
import { findUserOverlappingReservation, validateReservationRange } from "@/lib/drying";
...
export async function POST(req: Request) {
  ...
  const overlap = await findUserOverlappingReservation(
  ...
  if (overlap) {
    return NextResponse.json({ error: "该时段与您已有预约冲突" }, { status: 409 });
  }
  const check = await validateReservationRange(parsed.data.listingId, start, end);
  if (!check.ok) {
    return NextResponse.json(
      { error: check.message },
      { status: check.conflict ? 409 : 400 },
    );
  }

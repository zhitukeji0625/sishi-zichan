...
import { parseDateOnlyInput, validateReservationRange } from "@/lib/drying";
...
export async function POST(req: Request) {
  ...
  const start = parseDateOnlyInput(parsed.data.startDate);
  const end = parseDateOnlyInput(parsed.data.endDate);
  if (end < start) {

...
import { hasUserOverlappingReservation, validateReservationRange } from "@/lib/drying";
...
export async function POST(req: Request) {
  ...
  const duplicate = await hasUserOverlappingReservation(parsed.data.listingId, user.id, start, end);
  if (duplicate) {
    return NextResponse.json({ error: "您已有重叠时段的预约" }, { status: 409 });
  }

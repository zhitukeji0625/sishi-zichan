...
export async function POST(req: Request) {
  ...
  const latestBookable = startOfDay(addDays(new Date(), maxAdvanceDays));
  if (start > latestBookable || end > latestBookable) {
    return NextResponse.json({ error: `最多可提前 ${maxAdvanceDays} 天预约` }, { status: 400 });
  }
  try {

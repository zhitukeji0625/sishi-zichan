...
import { validateBookingWindow, validateReservationRange } from "@/lib/drying";
...
export async function POST(req: Request) {
  ...
  try {
    const res = await prisma.$transaction(async () => {
      const capacityCheck = await validateReservationRange(parsed.data.listingId, start, end);
      if (!capacityCheck.ok) {
        throw new Error(capacityCheck.message);
      }
      return prisma.dryingReservation.create({
        data: {

...
Vitest 会尝试连接数据库（未配置 `.env` 时默认 `mysql://root:root@127.0.0.1:3306/sishi`，与 `vitest.config.ts` 一致）。若当前无法连接 MySQL/MariaDB，**出价相关的集成测试会自动跳过**，命令仍返回成功。在 CI 或需要强制跑通数据库用例时：

```bash
VITEST_REQUIRE_DB=1 npm run test
```

运行 `npx prisma db push`、`npm run db:seed` 时需在环境中提供 `DATABASE_URL`（通常由根目录 `.env` 提供，可从 `.env.example` 复制）。

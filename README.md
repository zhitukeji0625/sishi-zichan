...
默认运行**不依赖数据库**的规则单测；若要对 `placeBid` 做 Prisma 集成测试，请复制 `.env.test.example` 为 `.env.test`（或配置根目录 `.env` 的 `DATABASE_URL`）并确保数据库已 `prisma db push`。
...
- 构建阶段仍跳过 ESLint（`next.config.ts`）；本地请运行 `npm run lint`（使用 `eslint.config.mjs` 与 `eslint-config-next` 的扁平配置）。

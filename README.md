...
- 无可用数据库时：仍会运行 `computeMinimumNextBid` 等**纯规则单测**；依赖 Prisma 的竞拍出价集成测试会自动跳过（控制台会有提示）。
- 本地已按 `AGENTS.md` 启动 MariaDB 并完成 `prisma db push` 后，同一条命令会跑完全部用例。
- CI 若需「无库则失败」，可设置环境变量 `VITEST_REQUIRE_DB=1`。

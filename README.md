...
默认 `npm run test` 不连数据库；若已启动 `docker compose` 中的 MySQL 并完成 `prisma db push`，可运行完整出价集成测试：
...
- 使用 ESLint 9 扁平配置（`eslint.config.mjs` + `eslint-config-next`），`npm run lint` 与 `next build` 会执行类型检查与 ESLint。

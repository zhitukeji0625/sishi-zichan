...
Vitest 会运行 `src/lib/__tests__/` 下的用例。依赖 Prisma 的出价集成测试在 **能连上数据库** 时执行：未配置 `.env` 时默认尝试 `mysql://root:root@127.0.0.1:3306/sishi`（与上文 Docker MariaDB 一致）；若无法连接则跳过相关用例并打印提示。CI 中若希望数据库不可用时 **直接失败**（而非跳过），可设置环境变量 `VITEST_REQUIRE_DB=1`。

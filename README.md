...
- 默认会尝试连接 `DATABASE_URL`（未配置时与 `vitest.config.ts` 一致，假定本机 Docker MariaDB：`mysql://root:root@127.0.0.1:3306/sishi`）。**连不上库时**会跳过数据库集成用例并打印提示，退出码仍为 0。
- CI 或本地需**强制**跑通含库的集成测试时：

```bash
npm run test:ci
```

（需已 `npx prisma db push` 且数据库可连。）

...
/** Idempotent dict seed — safe to run on every `db:seed`. */
export async function seedDictCategories(prisma: PrismaClient) {
  for (const cat of dictCategories) {
    ...
    console.log(`  Dict: created ${cat.code}`);
  }
}

/** Keep demo user's auction project LIVE for manual / H5 testing. */
export async function ensureDemoAuction(prisma: PrismaClient) {

...
export const dictSeedCategories = [
  {
    code: "asset_type",
    name: "资产类型",
...
/** Idempotent: creates built-in dict categories when missing. */
export async function seedDict(client: PrismaClient = new PrismaClient()) {
  for (const cat of dictSeedCategories) {

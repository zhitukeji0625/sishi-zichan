...
/** 幂等写入内置数据字典（可由 seed.ts 或单独 tsx 调用） */
export async function seedDictCategories(client: PrismaClient = prisma) {
...
const runStandalone = process.argv.some((a) => a.includes("seed-dict"));
if (runStandalone) {
  main()

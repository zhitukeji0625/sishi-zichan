...
/** 旧库中竞拍可能已 ENDED；保证演示用户始终有可出价的 LIVE 项目 */
async function ensureDemoLiveAuction() {
  ...
  const asset =
    (await prisma.asset.findFirst({ where: { status: AssetStatus.IDLE }, orderBy: { createdAt: "desc" } })) ??
  ...
  console.log(`Ensured demo LIVE auction ${project.code} for 13800138000`);
}

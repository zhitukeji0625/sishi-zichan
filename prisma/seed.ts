...
/** Keep demo auction LIVE so mobile bidding flows stay testable after cron/seed reruns. */
async function refreshDemoAuction() {
  ...
  if (!project) {
    project = await prisma.auctionProject.findFirst({ orderBy: { updatedAt: "desc" } });
    if (!project) return;

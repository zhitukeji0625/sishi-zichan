...
/** Keep demo auction in LIVE window when DB was seeded long ago. */
async function refreshDemoAuction() {
  ...
  console.log("Demo auction refreshed (LIVE, +7d).");
}
...
async function main() {
  ...
  if (existing > 0) {
    ...
    console.log("Seed skipped: data already present (dict + demo auction updated).");
    return;
  }

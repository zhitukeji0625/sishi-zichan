...
/** Ensure demo user can bid: create a LIVE project when none exists (e.g. after cron ends old demos). */
async function ensureDemoLiveAuction() {
  ...
  const asset =
    ...
    (await prisma.asset.findFirst({ where: { type: { not: AssetType.DRYING_FIELD } } }));
  if (!asset) return;

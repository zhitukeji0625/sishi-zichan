...
/** 演示竞拍过期后，每次 seed 仍将其刷新为 LIVE，便于功能测试与演示。 */
async function ensureDemoAuctionLive() {
  ...
  const projectId = reg?.projectId ?? (await prisma.auctionProject.findFirst({ orderBy: { createdAt: "desc" } }))?.id;
  if (!projectId) return;
  ...
  console.log("Demo auction refreshed to LIVE:", projectId);
}

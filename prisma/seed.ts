...
/** Refresh demo auction to LIVE when ended or ending within 1 hour. */
async function refreshDemoAuction() {
  ...
  const needsRefresh =
    project.status === "ENDED" || endsAt - now < 60 * 60 * 1000;
  ...
  console.log("Demo auction refreshed to LIVE:", project.id);
}

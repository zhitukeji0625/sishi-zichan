...
/** 演示竞拍过期后 cron 会标为 ENDED；重复 seed 时恢复一条可出价演示项目 */
async function refreshDemoAuction() {
  ...
  await prisma.auctionRegistration.upsert({
    where: {
      projectId_endUserId: { projectId: candidate.id, endUserId: demoUser.id },
    },
  ...
  console.log(`Demo auction refreshed: ${candidate.code} -> LIVE`);
}

...
/** Keep demo user able to bid when seed is re-run after an auction ended. */
async function ensureDemoLiveAuction() {
  ...
  const stale = await prisma.auctionProject.findFirst({
    where: { assetId: asset.id, status: { in: ["LIVE", "ENDED", "SCHEDULED"] } },
  ...
  console.log("Demo LIVE auction refreshed for 13800138000.");
}

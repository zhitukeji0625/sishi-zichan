...
/** Keep demo auction in LIVE window so README demo account can bid after re-seed. */
async function refreshDemoAuction() {
  ...
  console.log(`Demo auction ${reg.projectId} refreshed (LIVE until ${ends.toISOString()}).`);
}

async function main() {
  const existing = await prisma.auctionProject.count();
  if (existing > 0) {
    console.log("Seed skipped: data already present. Refreshing dict + demo auction…");

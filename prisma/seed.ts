...
/** Keep demo auction LIVE for testing when it has ended or is about to end. */
async function refreshDemoAuctionProject() {
  ...
  const needsRefresh =
    ...
    reg.project.endsAt.getTime() - now < oneHour;
  ...
  console.log("Refreshed demo auction project to LIVE (+7 days).");
}
...
async function main() {
  ...
  if (existing > 0) {
    await refreshDemoAuctionProject();

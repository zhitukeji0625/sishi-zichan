...
async function ensureDemoAuction() {
  ...
  let project = await prisma.auctionProject.findFirst({ where: { code: "DEMO_LIVE_AUCTION" } });
  if (!project) {
    project = await prisma.auctionProject.findFirst({ orderBy: { createdAt: "asc" } });
  }
  if (!project) return;

  await prisma.auctionProject.update({
    where: { id: project.id },
    data: { code: "DEMO_LIVE_AUCTION", status: "LIVE", startsAt: starts, endsAt: ends },
  });

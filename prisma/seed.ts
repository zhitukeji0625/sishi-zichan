...
/** Keep demo auction project in a biddable window for 13800138000. */
async function ensureDemoAuction() {
  ...
  const now = Date.now();
  const needsRefresh =
    reg.project.status === "ENDED" ||
    reg.project.endsAt.getTime() <= now ||
    reg.project.startsAt.getTime() > now + 60_000;
  ...
  console.log("Demo auction refreshed for 13800138000.");
}
...
async function main() {
  ...
  if (existing > 0) {
    ...
    console.log("Seed skipped: data already present (dict + demo auction ensured).");
    return;
  }

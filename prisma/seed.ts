...
/** Ensure demo user has an active LIVE auction for testing and demos. */
async function refreshDemoAuction() {
  const demoUser = await prisma.endUser.findUnique({ where: { phone: "13800138000" } });
  if (!demoUser) {
    console.log("Demo user not found, skip auction refresh.");
    return;
  }

  const now = new Date();
  const active = await prisma.auctionRegistration.findFirst({
    where: {
      endUserId: demoUser.id,
      status: "APPROVED",
      depositPaid: true,
      project: { status: "LIVE", endsAt: { gt: now } },
    },
  });
  if (active) {
    console.log("Demo LIVE auction already active.");
    return;
  }

  const asset = await prisma.asset.findFirst({ where: { status: AssetStatus.IDLE } });
  if (!asset) {
    console.log("No idle asset for demo auction.");
    return;
  }

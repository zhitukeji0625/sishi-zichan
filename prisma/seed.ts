...
/** Ensure at least one LIVE auction exists for demo/testing. */
async function ensureDemoLiveAuction() {
  ...
  if (!asset) {
    console.warn("ensureDemoLiveAuction: no LAND asset found, skipping.");
    return;
  }
  ...
  const ended = await prisma.auctionProject.findFirst({
    where: { status: "ENDED", assetId: asset.id },
  ...
  console.log(`ensureDemoLiveAuction: restored LIVE project ${project.code}`);
}

...
/** 确保演示用户有可参与的 LIVE 竞拍（已有未过期 LIVE 则跳过） */
async function refreshDemoAuction() {
  const demoUser = await prisma.endUser.findUnique({ where: { phone: "13800138000" } });
  if (!demoUser) return;

  const now = new Date();
  const liveReg = await prisma.auctionRegistration.findFirst({
    where: {
  ...
  if (liveReg) {
    console.log("Demo auction: existing LIVE project, skip.");
    return;
  }

  const asset = await prisma.asset.findFirst({
    where: { status: AssetStatus.IDLE, type: AssetType.LAND },
    orderBy: { createdAt: "asc" },
  });
  if (!asset) {
    console.log("Demo auction: no idle land asset, skip.");
    return;
  }
  ...
  console.log(`Demo auction: created LIVE project ${project.code}`);
}

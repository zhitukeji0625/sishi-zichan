...
/** Reset demo auction to LIVE when seed is skipped but the project has ended or expired. */
async function refreshDemoAuction() {
  ...
  const now = Date.now();
  const expired = project.endsAt.getTime() <= now;
  const needsReset =
    project.status !== "LIVE" || expired || project.bids.length > 0;

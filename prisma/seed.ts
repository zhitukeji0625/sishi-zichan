...
async function ensureDemoLiveAuction() {
  ...
  let asset = await prisma.asset.findFirst({ where: { status: "IDLE" } });
  if (!asset) {
    const org = await prisma.organization.findFirst({ where: { level: "REGIMENT" } });
    if (!org) return;
  ...
  console.log("Demo LIVE auction created:", project.id);
}

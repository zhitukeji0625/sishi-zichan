...
/** 无进行中竞拍时，为演示账号创建一场 LIVE 项目（种子跳过或历史项目已结束时仍可演示出价）。 */
async function ensureDemoLiveAuction() {
  ...
  let asset = await prisma.asset.findFirst({
    where: { type: { in: [AssetType.LAND, AssetType.WORKSHOP, AssetType.FACILITY] } },
  ...
  console.log(`ensureDemoLiveAuction: created LIVE project ${project.code} for ${DEMO_USER_PHONE}`);
}

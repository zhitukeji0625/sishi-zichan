...
/** Keep demo auction in LIVE state for dev / smoke tests when DB already has seed data. */
async function refreshDemoAuction() {
  const demoUser = await prisma.endUser.findUnique({ where: { phone: "13800138000" } });
  if (!demoUser) return;

  const asset =
    (await prisma.asset.findFirst({ where: { type: AssetType.LAND }, orderBy: { createdAt: "desc" } })) ??
    (await prisma.asset.findFirst({ orderBy: { createdAt: "desc" } }));
  if (!asset) return;
  ...
  console.log(`Demo auction refreshed: ${project.id} (LIVE)`);
}

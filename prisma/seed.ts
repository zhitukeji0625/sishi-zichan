...
/** Ensure a LIVE demo auction exists (idempotent — for smoke tests after prior auctions ended). */
export async function ensureDemoLiveAuction() {
  ...
  if (!asset) {
    const reg = await prisma.organization.findFirst({ where: { level: OrgLevel.REGIMENT } });
    if (!reg) return null;
  ...
  console.log(`ensureDemoLiveAuction: created LIVE project ${project.code}`);
  return project;
}

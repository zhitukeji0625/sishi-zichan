...
async function refreshDemoAuction() {
  ...
  if (!reg) return;
  const now = Date.now();
  const endsAt = reg.project.endsAt.getTime();
  if (reg.project.status === "LIVE" && endsAt > now + 60 * 60 * 1000) return;
  ...
  console.log("Demo auction refreshed to LIVE until", ends.toISOString());
}
...
async function main() {
  ...
  if (existing > 0) {
    ...
    console.log("Seed skipped: data already present (demo auction dates refreshed if stale).");
    return;
  }

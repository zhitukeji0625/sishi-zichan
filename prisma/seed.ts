...
/** Ensure demo user can bid on a LIVE auction (refresh ended projects). */
async function ensureDemoAuction() {
  ...
  const recent = await prisma.auctionProject.findFirst({
  ...
  if (recent) {
    ...
    await prisma.auctionRegistration.upsert({
      where: {
        projectId_endUserId: { projectId: recent.id, endUserId: demoUser.id },
      },
    ...
    console.log(`Demo auction refreshed: ${recent.code}`);
    return;
  }
...
async function main() {
  ...
  if (existing > 0) {
    console.log("Base seed skipped: data already present.");

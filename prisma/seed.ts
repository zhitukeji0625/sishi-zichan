...
/** Ensure demo user can bid when no LIVE auction exists (e.g. after prior demo ended). */
async function ensureDemoLiveAuction() {
  ...
  const asset = await prisma.asset.findFirst({ where: { status: AssetStatus.IDLE, type: AssetType.LAND } });

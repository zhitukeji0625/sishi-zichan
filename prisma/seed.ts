...
/** Keep demo auction bid-able when seed runs against an existing database. */
async function ensureDemoAuctionLive() {
  ...
  const project =
    (reg &&
      (await prisma.auctionProject.findUnique({ where: { id: reg.projectId } }))) ||

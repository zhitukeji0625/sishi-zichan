...
/** 确保存在可出价的演示竞拍（过期则刷新或新建） */
export async function ensureDemoAuction(client: PrismaClient = prisma) {
  const now = new Date();
  const live = await client.auctionProject.findFirst({
    where: { status: "LIVE", endsAt: { gt: now } },
  });
  if (live) {
    console.log(`Demo auction already live: ${live.code}`);
    return live;
  }
  ...
  const existing = await client.auctionProject.findFirst({ orderBy: { createdAt: "desc" } });
  if (existing) {
    const project = await client.auctionProject.update({
  ...
  const asset = await client.asset.findFirst({ where: { type: AssetType.LAND } });
  if (!asset) {
    console.log("No land asset found; cannot create demo auction.");
    return null;
  }

  const project = await client.auctionProject.create({
    data: {
      code: `AP${Date.now()}`,
      assetId: asset.id,
      startPrice: 8000,
  ...
  if (demoUser) {
    await client.auctionRegistration.create({
      data: {
        projectId: project.id,
        endUserId: demoUser.id,
        status: "APPROVED",
...
async function main() {
  ...
  if (existingOrgs > 0) {
    await ensureDemoAuction(prisma);
    console.log("Seed skipped: core data already present.");
    return;
  }

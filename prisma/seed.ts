...
async function refreshDemoAuctionIfStale() {
  ...
  const project = reg?.project ?? (await prisma.auctionProject.findFirst({ orderBy: { createdAt: "asc" } }));
  if (!project) return;

  const now = Date.now();
  const hasResult = !!(await prisma.auctionResult.findUnique({ where: { projectId: project.id } }));
  const stale =
    project.status !== "LIVE" ||
    project.endsAt.getTime() <= now + 60_000 ||
    (project.status === "LIVE" && hasResult);
  if (!stale) return;

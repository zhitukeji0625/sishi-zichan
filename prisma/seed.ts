...
/** Keep at least one LIVE auction for demo / integration tests when prior projects expired. */
async function ensureDemoLiveAuction() {
  ...
  let asset = await prisma.asset.findFirst({ where: { name: "团部东侧闲置地块" } });
  if (!asset) {
    asset = await prisma.asset.findFirst({ where: { type: AssetType.LAND } });
  }
  if (!asset) return;

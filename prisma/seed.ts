...
/** Keep demo auction LIVE when seed re-runs on an existing DB (projects may have ended). */
async function ensureDemoLiveAuction() {
  ...
  if (reg?.project) {
    ...
    console.log("Refreshed demo auction to LIVE (existing project).");
    return;
  }
  ...
  console.log("Created new demo LIVE auction project.");
}

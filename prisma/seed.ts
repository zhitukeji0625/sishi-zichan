...
/** 演示账号需始终关联一场进行中的竞拍；重复执行 seed 时刷新时间与状态。 */
async function refreshDemoAuction() {
  ...
  await prisma.auctionBid.deleteMany({ where: { projectId: reg.projectId } });
  ...
  console.log(`Demo auction refreshed: ${reg.project.code} (LIVE until ${ends.toISOString().slice(0, 10)})`);
}

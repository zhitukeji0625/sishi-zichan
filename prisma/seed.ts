...
async function refreshDemoAuction() {
  ...
  if (!isLive) {
    ...
    console.log(`  Demo auction ${project.code} already LIVE; cleared bids`);
  }
...
async function main() {
  ...
  if (existing > 0) {
    console.log("Business data present; refreshing demo auction...");
    await refreshDemoAuction();
    console.log("Seed OK (incremental).");
    return;
  }

...
async function renewDemoAuctionIfNeeded() {
  ...
  if (!project) return;
  const expired = project.status === "ENDED" || project.endsAt.getTime() < Date.now();
  if (!expired) return;
  ...
  console.log(`Renewed demo auction ${project.code} (${project.asset.name}) → LIVE until ${ends.toISOString()}`);
}

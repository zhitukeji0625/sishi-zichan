...
/** Keep demo auction in LIVE state for README / smoke tests. */
async function refreshDemoAuction() {
  ...
  let project = await prisma.auctionProject.findUnique({ where: { code: DEMO_AUCTION_CODE } });
  if (!project) {
  ...
  await prisma.auctionProject.update({
    where: { id: project.id },
    data: { code: DEMO_AUCTION_CODE, status: "LIVE", startsAt: starts, endsAt: ends },
  });
  ...
  console.log(`Demo auction refreshed (${DEMO_AUCTION_CODE}, ends ${ends.toISOString()}).`);
}

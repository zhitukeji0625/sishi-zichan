...
async function main() {
  ...
  if (existing > 0) {
    ...
    const demoProject = await prisma.auctionProject.findFirst({ orderBy: { createdAt: "desc" } });
    ...
    console.log("Seed skipped: data already present (demo auction dates refreshed).");
    return;
  }

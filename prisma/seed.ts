...
/** Reset demo auction to LIVE so cron tests can bid. Clears stale result/bids. */
export async function refreshDemoAuction(prismaClient: PrismaClient = prisma) {
  const project = await prismaClient.auctionProject.findFirst({
  ...
  await prismaClient.auctionBid.deleteMany({ where: { projectId: project.id } });
  await prismaClient.auctionResult.deleteMany({ where: { projectId: project.id } });

  await prismaClient.auctionProject.update({
    where: { id: project.id },
    data: { status: "LIVE", startsAt: starts, endsAt: ends },
  });

  const demoUser = await prismaClient.endUser.findUnique({ where: { phone: "13800138000" } });
  if (demoUser) {
    await prismaClient.auctionRegistration.upsert({
      where: {

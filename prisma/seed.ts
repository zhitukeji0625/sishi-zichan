...
/** Reset the first auction project to LIVE for demo/testing. */
export async function refreshDemoAuction() {
  ...
  await prisma.auctionProject.update({
    where: { id: project.id },
    data: { status: "LIVE", endsAt, startsAt: new Date(Date.now() - 60_000) },
  });
  ...
  if (demoUser) {
    await prisma.auctionRegistration.upsert({
      ...
      create: { projectId: project.id, endUserId: demoUser.id, status: "APPROVED", depositPaid: true },
    });
  }
  console.log(`Demo auction ${project.code} reset to LIVE (ends ${endsAt.toISOString()})`);
}
...
async function main() {
  ...
  if (existing > 0) {
    ...
    console.log("Seed skipped: data already present. Dict refreshed, demo auction reset.");
    return;
  }

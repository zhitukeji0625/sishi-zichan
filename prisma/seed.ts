...
/** Keep the demo auction LIVE so the seeded user can bid after re-seeding. */
async function refreshDemoAuction() {
  ...
  const target = regs.find(
    (r) =>
      r.project.status === "ENDED" ||
      r.project.status === "CANCELLED" ||
      r.project.endsAt.getTime() <= now,
  );
  if (!target) return;
  ...
  console.log(`Refreshed demo auction ${target.project.code} to LIVE.`);
}

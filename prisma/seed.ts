...
/** Keep the demo auction testable when it has ended or is about to end. */
async function refreshDemoAuction() {
  const project = await prisma.auctionProject.findFirst({ orderBy: { createdAt: "asc" } });
  if (!project) return;
  const oneHour = 60 * 60 * 1000;
  const needsRefresh =
    project.status === "ENDED" || project.endsAt.getTime() - Date.now() < oneHour;
  ...
  console.log(`Refreshed demo auction ${project.code} to LIVE (ends ${ends.toISOString()}).`);
}

...
/** Keep demo auction playable when seed data already exists but the project has ended. */
async function refreshDemoAuctionIfNeeded() {
  ...
  const now = new Date();
  if (reg.project.endsAt > now && reg.project.status !== "CANCELLED") return;

  const starts = new Date(Date.now() - 60 * 1000);
  const ends = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
  await prisma.$transaction([
    prisma.auctionBid.deleteMany({ where: { projectId: reg.projectId } }),
    prisma.auctionResult.deleteMany({ where: { projectId: reg.projectId } }),
    prisma.auctionProject.update({
      where: { id: reg.projectId },
      data: { startsAt: starts, endsAt: ends, status: "LIVE" },
    }),
  ]);
  console.log(`Demo auction refreshed: ${reg.project.code}`);
}

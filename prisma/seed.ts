...
/** 保证演示账号有可出价的进行中竞拍（旧库竞拍过期后 seed 仍可修复演示数据）。 */
async function ensureDemoLiveAuction() {
  ...
  console.log(`Ensured demo LIVE auction ${project.code} (${project.id}).`);
}
...
async function main() {
  ...
  if (existing > 0) {
    ...
    console.log("Seed: existing data kept; demo live auction checked.");
    return;
  }

...
/** 演示账号需有可出价竞拍；旧库中项目可能已 ENDED，每次 seed 时补齐 */
async function ensureDemoLiveAuction() {
  const demoUser = await prisma.endUser.findUnique({ where: { phone: "13800138000" } });
  if (!demoUser) return;

  const activeForDemo = await prisma.auctionProject.findFirst({
    where: {
  ...
  console.log("Ensured demo LIVE auction for 13800138000:", project.code);
}

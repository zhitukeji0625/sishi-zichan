...
import { ensureDemoLiveAuction } from "../src/lib/demo-auction";
...
async function main() {
  ...
  if (existing > 0) {
    const live = await ensureDemoLiveAuction();
    await seedDict(prisma);
    console.log(
      live
        ? `Seed refresh OK. Demo auction renewed: ${live.code}`
        : "Seed refresh OK (existing data).",
    );
    return;
  }
